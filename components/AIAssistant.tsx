...
export function AIAssistant({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const currentSchemaContext = nodes.length > 0 
        ? nodes
      ...
      const schema = await generateSchemaWithAI(prompt, apiKey, currentSchemaContext);
      ...
      const findExistingModel = (modelName: string) => {
        ...
        const node = nodes.find((n) => n.data?.model?.class === modelName || n.data?.table?.name === modelName.toLowerCase() + 's'); 
      ...
      schema.tables.forEach((table) => {
        ...
        if (!modelsProcessed.has(modelName)) {
            ...
            newFiles.push({
                name: existingModelFile ? existingModelFile.name : `${modelName}.php`,
                path: existingModelFile ? existingModelFile.path : `/app/Models/${modelName}.php`,
      ...
      <div className="bg-background border border-border shadow-2xl rounded-xl overflow-hidden flex flex-col">
        ...
        >
            ...
            <div className="flex items-center gap-1">
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); setShowSettings(!showSettings); }}>
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive" onClick={(e) => { e.stopPropagation(); onOpenChange(false); }}>
            ...
            <div className="p-4 bg-muted/10 border-b border-border/50 animate-in slide-in-from-top-2">
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        />
                        <Button size="sm" variant="outline" className="h-8" onClick={() => setShowSettings(false)}>Done</Button>
                    </div>
        ...
        {/* Main Input Area */}
        <div className={`p-4 transition-all duration-300 ${isExpanded ? 'h-64' : 'h-auto'}`}>
            <div className="relative h-full flex flex-col gap-3">
                <Textarea
                    ...
                    className={`resize-none bg-background font-mono text-sm border-0 focus-visible:ring-0 p-0 shadow-none ${isExpanded ? 'flex-1' : 'h-10 min-h-[40px]'}`}
                    value={prompt}
                ...
                <div className="flex justify-between items-center pt-2 border-t border-border/30">
                     <span className="text-[10px] text-muted-foreground">
                        {nodes.length > 0 ? `${nodes.length} existing tables in context` : "Start from scratch"}
                     </span>
                     <Button 
                        size="sm" 
